...
Page({
  ...
  applySync(data){if(!data.teacher||data.teacher.role!=='班主任'){if(this.permissionExitPending)return;this.permissionExitPending=true;if(this.alertEnabled&&wx.disableAlertBeforeUnload)wx.disableAlertBeforeUnload();wx.showModal({title:'班主任身份已转让',content:'你现在是该教室的普通任课教师，班级管理权限已经交接给新班主任。',showCancel:false,confirmText:'返回首页',success:()=>wx.switchTab({url:'/pages/home/index'})});return;}const needsSetup=!this.data.isCloudRoom&&data.classroomConfigured===false;const wasOnboarding=this.data.onboarding;this.currentTeacherId=data.teacher.connectionId;if(!this.hasInitialSync){this.hasInitialSync=true;this.setData({className:data.className||this.room.name||'',students:(data.students||[]).map(item=>({...item})),studentNames:(data.students||[]).map(item=>item.name)});}this.setData({onboarding:needsSetup,managingTeacherId:'',approvedTeachers:((data.teachers&&data.teachers.approved)||[]).map(item=>teacherView(item,this.currentTeacherId)),pendingTeachers:((data.teachers&&data.teachers.pending)||[]).map(item=>teacherView(item,this.currentTeacherId))});if(needsSetup&&!this.alertEnabled){this.alertEnabled=true;if(wx.enableAlertBeforeUnload)wx.enableAlertBeforeUnload({message:'完成班级基础配置后才能开始使用教室。'});}if(!needsSetup&&this.alertEnabled){this.alertEnabled=false;if(wx.disableAlertBeforeUnload)wx.disableAlertBeforeUnload();}if(wasOnboarding&&!needsSetup&&this.awaitingSetupSave){this.awaitingSetupSave=false;wx.showModal({title:'教室配置完成',content:'班级名称和学生名单已经保存，现在可以开始使用全部教学功能。',showCancel:false,confirmText:'进入首页',success:()=>wx.switchTab({url:'/pages/home/index'})});}this.applyFaces(data.pendingFaces||[]);},
  ...
  async editSubjects(event){const {id,name,subjects}=event.currentTarget.dataset;const current=String(subjects||'').split('、').filter(Boolean);const session=sessionStore.load();const available=session&&session.cloud&&session.cloud.availableSubjects;const values=await subjectOptions.choose(current,`设置${name}的授课科目`,available);if(!values)return;socket.send({type:'manage-teacher',action:'update',connectionId:id,subjects:values});},
