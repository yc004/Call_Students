...
global.wx = {
  request(options) {
    ...
    if (options.url.endsWith('/api/v1/auth/mini-program/login')) {
    ...
    if (options.url.endsWith('/api/v1/teacher/profile')) {
      options.success({ statusCode:200, data:{
...
(async () => {
  const cloud = await cloudApi.loginMiniProgramAccount({ serverUrl:'https://cloud.example.com', loginName:'teacher01', password:'default-pass', deviceName:'微信小程序' });
  ...
  assert.deepStrictEqual(calls[0].data, { loginName:'teacher01', password:'default-pass', deviceName:'微信小程序' });
  assert.ok(!Object.prototype.hasOwnProperty.call(calls[0].data, 'key'));

  const httpCloud = await cloudApi.loginMiniProgramAccount({ serverUrl:'192.168.1.20:8080', useHttps:false, loginName:'teacher01', password:'default-pass', deviceName:'微信小程序' });
