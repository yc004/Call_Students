(function (root, factory) {
  ...
  function deadlineLabel(key, now = Date.now()) {
    ...
    const target = parseLocalDateTime(`${key}T00:00`);
  ...
  function submissionSummary(assignment, students) {
    ...
    (students || []).forEach(student => {
      ...
      if (status === '已提交') result.submitted += 1;
      else if (status === '迟交') result.late += 1;
      else if (status === '未提交') result.pending += 1;
      else if (status === '免交') result.exempt += 1;
      else result.other += 1;
    });
    const required = Math.max(0, result.total - result.exempt);
    result.completed = result.submitted + result.late;
    result.rate = required ? Math.round(result.completed / required * 100) : 100;
    return result;
  }

  return { parseLocalDateTime, dateKey, stageOf, typeOf, deadlineLabel, groupByDeadline, submissionSummary };
});
