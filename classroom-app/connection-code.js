(function (root, factory) {
  ...
  function digit(payload){let sum=0;for(let i=payload.length-1,d=true;i>=0;i--,d=!d){let n=Number(payload[i]);if(d){n*=2;if(n>9)n-=9;}sum+=n;}return String((10-sum%10)%10);}
  ...
  function decode(code){const n=normalize(code);if(n.length!==9||n[8]!==digit(n.slice(0,8)))throw new Error('连接码无效');const index=Number(n.slice(0,8));const r=[...RANGES].reverse().find(x=>index>=x.offset);if(!r||index-r.offset>=r.size)throw new Error('连接码无效');return numberIp(ipNumber(r.base)+index-r.offset);}
